...
document.addEventListener('DOMContentLoaded', () => {
  console.log('[POC 2] Resume Review initialized');
  initSearch();
...
function initSearch() {
  ...
  const contextualInput = document.getElementById('resume-contextual-query');
  const runBtn = document.getElementById('btn-run-resume-analysis');
  const exportBtn = document.getElementById('btn-export-results');

  if (runBtn) {
    runBtn.addEventListener('click', async () => {
      console.log('[POC 2] Review Resume button clicked');
      runBtn.disabled = true;
      runBtn.textContent = 'Reviewing resume...';
      const q = contextualInput ? contextualInput.value.trim() : '';

      await triggerResumeReviewStepper(q || 'Review Resume Against Target Job Description');

      renderReviewPanel();

      runBtn.disabled = false;
      runBtn.textContent = 'Run Resume Review';

      showToast('Resume review completed: ATS score 86% matched with Job Description.', 'success');
    });
  }

  if (exportBtn) {
    exportBtn.addEventListener('click', () => {
      triggerResumeReviewStepper('Review Resume Against Target Job Description');
    });
  }
...
async function triggerResumeReviewStepper(customQuery = '') {
 const stepperCard = document.getElementById('resume-stepper-card');
 const stepperTitle = document.getElementById('resume-stepper-title');
 const stepperTimer = document.getElementById('resume-stepper-timer');
 const stepperList = document.getElementById('resume-stepper-steps-list');

 if (stepperCard) stepperCard.style.display = 'block';
 if (stepperTitle) stepperTitle.textContent = customQuery ? `AI Resume Review: "${customQuery}"` : 'Resume Analyzer Neural Engine Executing...';
 if (stepperList) stepperList.innerHTML = '';

 const steps = [
 'Initializing Resume Parser & ATS Matching v3.4...',
 'Extracting document sections, experience impact verbs, and technical keywords...',
 'Cross-referencing JD cutoff requirements (TechCorp AI Research Engineer)...',
 'Generating targeted phrasing improvements & ATS score breakdown...',
 'Resume Review Complete (ATS Score: 94 / 100 • 3 suggestions ready)'
 ];
 ...
 if (stepperTitle) stepperTitle.textContent = `✓ Resume Analysis Complete (${finalTime}s • 94/100 ATS Score)`;

 showToast('Resume ATS Audit Complete. 3 improvement suggestions ready.', 'success');
}
