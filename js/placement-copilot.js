...
function init() {
  console.log('[POC 4] Placement Assistant initialized');
  renderSessionsList();
  renderSession();
  initActionHandlers();
  console.log('[POC 4] initActionHandlers completed successfully');
}
...
function initSessionManagement() {
 ...
 if (!btnNewSession) {
   throw new Error('Missing required element #btn-new-session');
 }
 btnNewSession.addEventListener('click', createNewSession);

 if (!sessionSearchInput) {
   throw new Error('Missing required element #session-search-input');
 }
 ...
 if (!btnRenameSession) {
   throw new Error('Missing required element #btn-rename-session');
 }
 btnRenameSession.addEventListener('click', () => {
   const activeSession = sessions.find(s => s.id === activeSessionId);
   if (!activeSession) return;
   const newName = prompt('Rename Copilot Session:', activeSession.title);
   if (newName && newName.trim()) {
     activeSession.title = newName.trim();
     saveSessions();
     renderSessionsList();
     renderSession();
     showToast(`Renamed session to "${newName.trim()}".`, 'success');
   }
 });

 if (!btnClearSession) {
   throw new Error('Missing required element #btn-clear-session');
 }
 btnClearSession.addEventListener('click', () => {
   ...
   if (activeSession) {
     ...
     saveSessions();
     renderSession();
     showToast('Cleared session conversation history.', 'info');
   }
 });

 if (!btnDeleteSession) {
   throw new Error('Missing required element #btn-delete-session');
 }
...
function renderSessionsList() {
 ...
 const todaySessions = filtered.filter(s =>s.category === 'Today');
 const pastSessions = filtered.filter(s =>s.category !== 'Today');
...
<ul class="wf-sidebar-nav">
 ${todaySessions.map(renderSessionNavItem).join('')}
</ul>
</div>
 `;
 }
...
<ul class="wf-sidebar-nav">
 ${pastSessions.map(renderSessionNavItem).join('')}
</ul>
</div>
 `;
 }
 ...
 html = `<div style="font-size: 11px; color: var(--text-muted); padding: 8px;">No sessions found.</div>`;
 }
...
function renderSession() {
 ...
 if (!streamContainer) {
   throw new Error('Missing required element #chat-message-stream');
 }

 // Preserve empty-state node across re-renders (innerHTML would destroy it)
 if (emptyState && emptyState.parentNode === streamContainer) {
   streamContainer.removeChild(emptyState);
 }

 streamContainer.querySelectorAll('.wf-chat-bubble').forEach(node => node.remove());
 ...
 // Keep off-DOM reference alive by parking it on the document body as hidden
 // so getElementById still finds it on the next render.
 document.body.appendChild(emptyState);
 }

 const messagesHtml = activeSession.messages.map(m =>{
 if (m.sender === 'user') {
 return `
...
<button class="wf-btn-ghost wf-btn-xs btn-regen-msg"style="color: var(--text-muted);">Regenerate</button>
 ...
 streamContainer.insertAdjacentHTML('beforeend', messagesHtml);
...
function initPromptChips() {
 ...
 if (!container) {
   throw new Error('Missing required element #chat-prompt-chips');
 }
...
function initComposer() {
 ...
 if (!textarea) {
   throw new Error('Missing required element #chat-input-textarea');
 }
 if (!btnSend) {
   throw new Error('Missing required element #btn-send-chat');
 }
 if (!btnStop) {
   throw new Error('Missing required element #btn-stop-chat');
 }
 ...
 btnSend.addEventListener('click', () => {
   console.log('[POC 4] Send button clicked');
   const text = textarea.value.trim();
   if (text && !isStreaming) {
     textarea.value = '';
     handleSendMessage(text);
...
async function handleSendMessage(userText) {
 ...
 // Auto-create a session when none exists so conversation can begin immediately
 if (!activeSession) {
   const newId = `session-${Date.now()}`;
   activeSession = {
     id: newId,
...
function generateIntelligentResponse(userQuery) {
 ...
 linkText: 'Open Student Participation →',
 linkUrl: 'poc6-student-participation.html'
 },
 actionBtn: {
 text: 'View Participation Insights →',
 url: 'poc6-student-participation.html'
 }
 };
 }
...
function initContextRail() {
 ...
 if (!btnToggle) {
   throw new Error('Missing required element #btn-toggle-context-rail');
 }
 if (!rail) {
   throw new Error('Missing required element #context-rail-aside');
 }
