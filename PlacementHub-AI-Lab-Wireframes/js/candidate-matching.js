...
function initTopActions() {
  ...
  if (btnAnalyzeTop) {
    btnAnalyzeTop.addEventListener('click', async () => {
      showLoadingOverlay(document.body, 'Running AI neural skill alignment scoring...');
      await asyncSimulateApiCall(null, 1500);
      hideLoadingOverlay(document.body);

      // Recalculate AI scores dynamically
      candidates.forEach(c => {
        if (c.skills.includes('PyTorch') || c.skills.includes('React')) {
          c.status = 'Shortlisted';
        }
      });
      ...
      showToast('AI analysis completed. Evaluated 5 candidates, 4 strong matches shortlisted.', 'success');
    });
  }
}
...
function openCandidateDrawer(candidateId) {
  ...
  if (reasoningListEl) {
    reasoningListEl.innerHTML = cand.matchReasoning.map(r => `<div>${r}</div>`).join('');
  }
